...
After all GitOps components run successfuly, you can now run the workflow via Orchestrator.
...
4. **Tekton PipelineRuns are not successful**: Make sure that affinity-assistant is disabled in the TektonConfig CR. To disable it, you can run the following command: `oc patch tektonconfig config -n openshift-pipelines --type merge -p '{"spec":{"featureFlags":{"coschedule":false}}}'`
